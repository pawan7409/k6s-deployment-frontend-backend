...
### For azure-deploy.yaml
1. Go to: GitHub → Your Repository → Actions tab
2. On the left sidebar, select "Deploy to Azure Services"
3. Click the "Run workflow" button
4. Select deployment environment (production/staging/development)
5. Optionally enable "Apply Terraform changes"
6. Click "Run workflow" to execute

### For build-deploy.yml
1. Go to: GitHub → Your Repository → Actions tab
2. On the left sidebar, select "Build and Deploy"
3. Click the "Run workflow" button
4. Click "Run workflow" to execute

### For k8s-deploy.yaml
1. Go to: GitHub → Your Repository → Actions tab
2. On the left sidebar, select "Deploy to Kubernetes"
3. Click the "Run workflow" button
4. Click "Run workflow" to execute
...
- **Solution:** Cache fixed by removing invalid `cache-dependency-path`. Using just `cache: 'npm'` works correctly
...
**Issue:** Workflow triggers on path change but doesn't run
- **Solution:**
  1. Check that the file paths in `paths:` match your changes
  2. Verify you're pushing to the correct branch
  3. Ensure `workflow_dispatch:` is properly indented under `on:`
...
### Azure Secrets (for SERVICE_PRINCIPAL auth)
```bash
# Get these values from Azure CLI:
az account show --query id          # AZURE_SUBSCRIPTION_ID
az account show --query tenantId    # AZURE_TENANT_ID
az ad sp list --query "[?appDisplayName=='your-app-name'].appId" # AZURE_CLIENT_ID
```

### ACR Secrets (for Docker build)
```bash
# Get these from Azure Container Registry:
az acr credential show --name myregistry --query username
az acr credential show --name myregistry --query passwords[0].value
```

---

## Recent Fixes Applied (May 25, 2026)

✅ **azure-deploy.yaml**
- Fixed SERVICE_PRINCIPAL authentication with required `client-id`, `tenant-id`, `subscription-id`
- Added manual trigger support with `workflow_dispatch:` and environment selection
- Added comprehensive documentation in file header
- Fixed all 3 login steps (deploy-infrastructure, deploy-app-service, deploy-function-app)

✅ **tests.yaml & build-deploy.yml**
- Fixed npm cache error by removing invalid `cache-dependency-path: 'frontend/package-lock.json'`
- Cache now uses default npm cache directory
- Error resolved: "Some specified paths were not resolved, unable to cache dependencies"
